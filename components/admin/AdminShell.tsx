...
const LANDING_LINKS = [
  ...
  { href: "/admin/landing/marquee", label: "Marquee & Stats" },
];
  ...
  return (
    <div className="min-h-screen">
      <aside className="fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-white/8 bg-background p-6 overflow-y-auto">
        <Link href="/admin" className="heading text-xl text-zinc-200 block mb-1 shrink-0">
          HRS<span className="text-accent">.</span> CMS
        </Link>
        ...
        <div className="max-w-3xl mx-auto px-6 py-10">
          {!publicReadConfigured ? (
            <div className="mb-8 rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 text-sm text-amber-200/80">
              ...
              <code className="mono text-xs">PORTFOLIO_SPREADSHEET_ID</code> and the service
              account are configured in <code className="mono text-xs">.env.local</code>.
            </div>
          ) : null}

          <header className="mb-8">
            <h1 className="heading text-2xl md:text-3xl text-zinc-100">{title}</h1>
            {description ? (
